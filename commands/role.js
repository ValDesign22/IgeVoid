...
exports.run = (client, message, args, dbPrefixs, dbTickets, prefix) => {
    if (message.member.hasPermission("ADMINISTRATOR")) {
        if (args[0] === "create") {
            if (message.mentions.roles.first() || message.mentions.members.first() || message.mentions.channels.first()) {
                message.reply("Merci de me donner un nom de role et de ne pas mentionner un role existant, un membre ou un channel.")
            }
            else if (args[1]) {
                if(args[2]) {
                    const nom = args[1]
